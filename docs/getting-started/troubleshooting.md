...
| Hosted dashboard MCP calls return 403 `Origin is not allowed` | The MCP service rejected the browser `Origin`. Set `CHAINPAY_ALLOWED_ORIGINS` on **chainpay-mcp** to the exact frontend origin, with no trailing slash — for the hosted app that is `https://chainpay-frontend.onrender.com`. Then redeploy MCP. Do not use `*` to paper over this. `render.yaml` already lists the hosted origin plus local Vite; a Render dashboard override can still be empty or stale. |
| Browser console: `solana-client` is not allowed by Access-Control-Allow-Headers | The backend CORS layer must allow that header (or mirror the preflight). Redeploy **chainpay-backend** after that code is on `master`. A frontend-only deploy will keep SOL/USDC/PYUSD as Unavailable. |
