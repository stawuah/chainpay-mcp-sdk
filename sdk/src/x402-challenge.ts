...
/** Alias for agent/SDK callers: parse HTTP 402 headers and body into a detected challenge. */
export function detectPaymentChallenge(
  headers: { get(name: string): string | null },
  body: { parsed?: unknown },
...
export function parsePaymentRequiredFromResponse(
  ...
  return detectPaymentChallenge(headers, body, expectedResource);
}
...
export type UnsupportedSponsorResult = {
  action: "x402_unsupported_sponsor";
  mode: "unsupported-sponsor";
  protocol: typeof STANDARD_V2_PROTOCOL;
  protocolLabel: string;
  proofKind: "partially-signed-sponsored-transaction";
  network: typeof SOLANA_DEVNET_CAIP2;
  ...
  message: string;
  derivedRecipientTokenAccount?: string;
  mandateQuote?: {
...
export function unsupportedSponsorResult(
  option: StandardV2Option,
  extras?: Pick<UnsupportedSponsorResult, "derivedRecipientTokenAccount" | "mandateQuote">,
  ...
  return {
    ...
    reason: "facilitator_required",
    message:
      "Standard x402 v2 exact SVM requires a sponsor or facilitator to countersign a partially signed transaction. ChainPay settles through an on-chain mandate and returns a receipt-PDA proof. This challenge is quoted against your mandate; use pay.sh for facilitator merchants, or settleIfReceiptMerchant on allowlisted receipt merchants.",
    ...(extras?.derivedRecipientTokenAccount
      ? { derivedRecipientTokenAccount: extras.derivedRecipientTokenAccount }
      : {}),
    ...(extras?.mandateQuote ? { mandateQuote: extras.mandateQuote } : {}),
  };
}

export type MppUnsupportedResult = {
  action: "mpp_unsupported";
  mode: "unsupported-mpp";
  message: string;
...
export function parseMppWwwAuthenticate(value: string | null): MppUnsupportedResult | null {
  if (!value || !value.trim().toLowerCase().startsWith("payment")) return null;
  const trimmed = value.trim();
  const intent = trimmed.match(/\bintent="([^"]+)"/i)?.[1];
  const method = trimmed.match(/\bmethod="([^"]+)"/i)?.[1];
  const challengeId = trimmed.match(/\bid="([^"]+)"/i)?.[1];
  return {
    action: "mpp_unsupported",
    mode: "unsupported-mpp",
    message:
      "Machine Payments Protocol (MPP) uses WWW-Authenticate: Payment, not x402 PAYMENT-REQUIRED. ChainPay does not settle MPP sessions or Tempo/Stripe methods. Use pay.sh for MPP-gated APIs.",
    wwwAuthenticate: trimmed,
    ...(intent ? { intent } : {}),
    ...(method ? { method } : {}),
