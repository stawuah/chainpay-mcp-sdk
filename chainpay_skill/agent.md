...
- In **Mode 1.2 (Delegated Wallet):** The agent holds a distinct ephemeral keypair generated by the human in the browser. The agent signs autonomously, but the Anchor program enforces all limits.
...
Used when: Agent does not have a delegated keypair. Every payment requires human confirmation.
...
## 4. Agent Payment Flow — Mode 1.2 (Delegated Wallet)

Used when: Agent holds a delegated ephemeral keypair. Fully autonomous, no human confirmation per payment.

```
┌─────────┐     ┌──────────┐     ┌──────────┐     ┌──────────┐
│  Agent  │     │   MCP    │     │ Backend  │     │  Anchor  │
│(+keypair│     │          │     │          │     │          │
└────┬────┘     └─────┬────┘     └─────┬────┘     └─────┬────┘
     │                │                │                 │
     │ prepare_payment│                │                 │
...
**Key:** The delegated private key never enters MCP arguments, HTTP, backend
requests, logs, or platform memory. The agent runtime signs locally and sends
only the serialized signed transaction to the Axum backend.
